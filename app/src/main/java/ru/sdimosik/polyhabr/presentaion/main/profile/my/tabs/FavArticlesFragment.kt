...
import ru.sdimosik.polyhabr.databinding.FragmentFavArticlesBinding
import ru.sdimosik.polyhabr.domain.model.toUI
import ru.sdimosik.polyhabr.presentaion.main.feed.adapter.FeedAdapter

@AndroidEntryPoint
class FavArticlesFragment : BaseFragment(R.layout.fragment_fav_articles) {
    companion object {
        fun newInstance() = FavArticlesFragment()
    }

    private val binding by viewBinding(FragmentFavArticlesBinding::bind)

    private val viewModel by viewModels<FavArticlesViewModel>()
    ...
    private fun FragmentFavArticlesBinding.setup() {
        rvArticles.apply {
            adapter = feedAdapter
    ...
    private fun FragmentFavArticlesBinding.subscribe() {
